...
class UserCredentials(AbstractUser):
    ...
    def __str__(self):
        return '%s | %s %s %s | <Disabled> | %s' % (self.username, self.first_name, self.middle_name, self.last_name, self.dept_residence)
...
class UserTasks(models.Model):
    ...
    Task_UUID = models.UUIDField(max_length=32, default=uuid4, editable=False, unique=True, help_text="A Unique Identifier for the Tasks. This was used for inline verification of data deletion.")

    def __str__(self):
        return '%s | %s | %s |> Created as of %s' % (self.Task_Name, self.Task_Description, self.Task_Type, self.Task_CreateDate)